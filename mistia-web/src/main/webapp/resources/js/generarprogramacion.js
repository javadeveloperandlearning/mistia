...
function cargardatos() {
    ...
    // carga de distritos
    $.ajax({
        url: "rest/distrito/distritoslist.html",
        type: "POST",
        datatype: "json",
        success: function (data) {
            ...
            for (var i in data) {
                html += "<option value='" + data[i].codigoDistrito + "' >" + data[i].descripcion + "</option>";
            }
            //console.log(html);
            $("#cmbdistrito").html(html);
            $("#cmbdistrito").selectmenu({
                width: 300
            });
    ...
    $.ajax({
        url: "rest/tiposolicitud/tipoSolicitudlist.html",
        type: "POST",
        datatype: "json",
        success: function (data) {
            ...
            var html = "<option value = '0'>----Todos--- </option>";
            for (var i in data) {
                html += "<option value='" + data[i].codigoTipoSolicitud + "' >" + data[i].descripcion + "</option>";
            }

            $("#cmbtiposolicitud").html(html);
            $("#cmbtiposolicitud").selectmenu({
                width: 300
            });
...
function preload(titulo) {
    var html = "<div id='dialog-preload' title='"+titulo+"'>" +
            "<div  align='center' ><img src='/mistia-web/resources/images/preloader.gif'/></div>" +
            "</div>";
    $("#content").html(html);
...
$(document).ready(function () {
    ...
    var isshowls = true; // var para lista de solicitudes
    var isshowls2 = true;
    preload("Cargando solicitudes");
    cargardatos();

    // panel de solicitudes
    $("#btnmostrarcontentsoli").click(function(){
        if (isshowls) {
            $("#effect").hide("blind", {}, 1000, null);
            isshowls = false;
        } else {
            $("#effect").show("blind", {}, 500, null);
            isshowls = true;
        }
    });
      ...
      // panel de cuadrillas
    $("#btnlistarcuadrillas").click(function(){
        if (isshowls2) {
            $("#effect2").hide("blind", {}, 1000, null);
            isshowls2 = false;
        } else {
            $("#effect2").show("blind", {}, 500, null);
            isshowls2 = true;
        }
    });
    ...
    $("#btnmostrar").click(function () {
        ...
        //console.log($("#cmbdistrito").val());
        //console.log($("#cmbtiposolicitud").val());

        $("#dialog-preload").attr("title", "Cargando solicitudes!!!");
        $("#dialog-preload").dialog("open");
        cargarsolicitudes(null, $("#cmbdistrito").val(), $("#cmbtiposolicitud").val());
    ...
    // inicio de la config de la grilla
    $("#lstsolicitud").jqGrid({
        datatype: "local",
        height: 600,
        multiselect: true,
        ...
        colNames: ['Solicitud', 'Solicitud', 'Grupo', 'Prioridad', '', '', 'Tipo'],
        colModel: [
            {name: 'numeroSolicitud', index: 'numeroSolicitud', width: 0, sorttype: "int", sortable: false},
            {name: 'tag', index: 'tag', width: 120, sorttype: "text", align: "right", sortable: true},
            {name: 'descripcionGrupo', index: 'descripcionGrupo', width: 80, sorttype: "text", sortable: true},
            {name: 'prioridad', index: 'prioridad', width: 65, sorttype: "text", sortable: true},
        ...
        onSelectAll: function (aRowids, status) {
            ...
            /*for(var i in aRowids){
             console.log(" removiendo : "+aRowids[i]);
             var row = $("#lstsolicitud").jqGrid('getRowData',aRowids[i]);
             removeMark(row.numeroSolicitud);
             }*/
...
function cargarsolicitudes(grupos, codigoDistrito, codigoTipoSolicitud) {
    
    $("#lstsolicitud").jqGrid("clearGridData", true).trigger("reloadGrid");
    $.ajax({
        url: "rest/programacion/solicitudespendientes.html",
        data: {codigoDistrito: codigoDistrito, codigoTipoSolicitud: codigoTipoSolicitud},
        type: "POST",
        datatype: "json",
        success: function (data) {
            solicitudarray = data
