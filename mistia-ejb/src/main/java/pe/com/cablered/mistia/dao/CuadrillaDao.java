...
public class CuadrillaDao extends CrudDao<Cuadrilla> {
    ...
    public List<Cuadrilla> getCuadrillaLibresList(Date fecPrgn) {
        ...
        logger.info(" inicio metodo ; getCuadrillaLibresList ");
        ...
        try {
            String sql = "Select c from Cuadrilla c join c.vehiculo v "
                       + " where date_trunc('day', c.fechaProgramacion) =:pfecprgn"
                    + " and c.numeroCuadrilla not in  ( select  ca.numeroCuadrilla from Programacion p "
                    + "                                     join p.programacionDetalles pd "
                    + "                                     join pd.planTrabajo pt "
                    + "                                     join pt.cuadrilla ca where  date_trunc('day', p.fechaProgramacion) =:pfecprgn )  ";
            
            TypedQuery<Cuadrilla> query = getEntityManager().createQuery(sql, Cuadrilla.class);
            query.setParameter("pfecprgn", fecPrgn, TemporalType.TIMESTAMP);
            
            list = query.getResultList();
            
            logger.info(" fin metodo ; getCuadrillaLibresList ### "+ list.size());
