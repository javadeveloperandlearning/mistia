...
public class PlanTrabajoDetalleDao extends CrudDao<PlanTrabajoDetalle> {
    ...
    public List<PlanTrabajoDetalle> getPlanTrabajoDetalleList(Long numeroPlanTrabajo, Integer estadoProgramacion) {
        ...
        logger.info(" estadoProgramacion : " + estadoProgramacion);
    ...
    public Response transferir(PlanTrabajoDetalle planTrabajoDetalle) {

        Response response = new Response(Response.ERROR, Response.MSG_ERROR);

        try {

            String sql = "update  PlanTrabajoDetalle p set p.indTrnf =:pindtrnf   "
                    + "  where p.id  = :pid";
            Query query = getEntityManager().createQuery(sql);
            query.setParameter("pindtrnf", 1);
            query.setParameter("pid", planTrabajoDetalle.getId());
            query.executeUpdate();
