...
public class GrupoAtencionDetalleDao extends CrudDao<GrupoAtencionDetalle> {
    ...
    public List<GrupoAtencionDetalle> getGrupoAtencionDetalleListPorProgramacion(Long numeroProgramacion) {
        ...
        String sql = "Select   new GrupoAtencionDetalle(a.grupoAtencion, s) "
                ...
                + " where pd.programacion.numeroProgramacion=:pnumeroProgramacion"
                 + "  and (a.indTrnf is null or a.indTrnf = 0)";
    ...
    public Response transferir(GrupoAtencionDetalle grupoAtencionDetalle) {

        Response response = new Response(Response.ERROR, Response.MSG_ERROR);

        try {

            String sql = "update  GrupoAtencionDetalle p set p.indTrnf =:pindtrnf   "
                    + "  where p.id  = :pid";
            Query query = getEntityManager().createQuery(sql);
            query.setParameter("pindtrnf", 1);
            query.setParameter("pid", grupoAtencionDetalle.getId());
            query.executeUpdate();
