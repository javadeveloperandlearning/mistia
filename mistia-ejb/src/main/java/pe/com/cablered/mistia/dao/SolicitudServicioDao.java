...
public class SolicitudServicioDao extends CrudDao<SolicitudServicio> {
    ...
    public SolicitudServicio getSolicitud(long numeroSolicitud) {
        ...
        try {
            String sql = "Select new SolicitudServicio (s.numeroSolicitud,"
                    ...
                    + "s.poste, "
                    + "s.tipoSolicitud, "
                    + "s.latitud, s.longitud) from SolicitudServicio s where s.numeroSolicitud =:pnumeroSolicitud ";
    ...
    public List<SolicitudServicio> getSolicitudListPorEstado(int codigoEstado) {
        ...
        try {
            String sql = "Select new SolicitudServicio (s.numeroSolicitud,"
                    ...
                    + " join s.estado e where e.codigoEstado  =:pcodigoestado "
    ...
    public List<SolicitudServicio> getSolicitudList(int codigoEstado, Integer codigoDistrito, Integer codigoTipoSolicitud) {
       ...
       logger.info(" codigoCliente :" + codigoCliente);
        ...
        try {
            String sql = "Select  s.numeroSolicitud, "
                    ...
                    + "   s.fechaCreacion as fecharegistro, "
                    + "   c.codigoCliente, "
                    + "   c.tipoDocumento.codigoTipo as codigoTipoDocumento,"
            ...
            for (Object[] result : list) {
                ...
                Integer codigoTipoDocumento =  (Integer) result[5];
                if(codigoTipoDocumento == 11){
                    map.put("nomcliente", result[7]);
                }else{
                    map.put("nomcliente", result[6]);
                }
                ...
                map.put("tag", Util.getTag((SolicitudServicio) result[8]));

                lista.add(map);
            }

           /* String _sql = "select  new SolicitudServicio(s.numeroSolicitud) from SolicitudServicio s";
            TypedQuery<SolicitudServicio> _query = getEntityManager().createQuery(_sql, SolicitudServicio.class);
            List<SolicitudServicio> _list = _query.getResultList();
            for (SolicitudServicio solicitudServicio : _list) {

                Query queryu = getEntityManager().createQuery("update SolicitudServicio s set s.cliente.codigoCliente =:pcodigocliente where s.numeroSolicitud =:pnumerosolicitud");
                queryu.setParameter("pcodigocliente", Util.randInt(1, 26));
                queryu.setParameter("pnumerosolicitud", solicitudServicio.getNumeroSolicitud());
    ...
    public SolicitudServicio getSolicitudServicio1(Long numeroSolicitud) {
        ...
        try {
            ...
            TypedQuery<SolicitudServicio> query = getEntityManager().createQuery(sql, SolicitudServicio.class);
            query.setParameter("pnumerosolicitud", numeroSolicitud);
            List<SolicitudServicio> list = query.getResultList();
            
            if(list!=null && !list.isEmpty()){
                solicitudServicio =  list.get(0);
            }
